...
- **Future migration target:** the SDK now ships `client.beta.messages.tool_runner` with `anthropic.lib.tools.mcp.async_mcp_tool` for first-class MCP integration. Once that surface is GA-promoted out of `beta`, retire the manual `mcp-client-2025-11-20` beta-header path in `call_api_with_tool_loop`. (Last checked anthropic 0.109.1 on 2026-06-10 — still beta-only: `client.messages.tool_runner` does not exist, only `client.beta.messages.tool_runner`. 0.108.0 added `claude-fable-5`/`claude-mythos-5` to the typed Model union plus refusal-fallback machinery. Recheck on next anthropic SDK minor bump.)
- **Refusal fallback (server-side, beta):** when the requested model is in `REFUSAL_FALLBACK_MODELS` (`util.py`, currently `claude-fable-5`), `call_api_with_tool_loop` adds the `server-side-fallback-2026-06-01` beta and `fallbacks=[{"model": REFUSAL_FALLBACK_MODEL}]` (`claude-opus-4-8`, the only supported target at launch). Served-by detection reads `usage.iterations` for a `fallback_message` entry (covers sticky-routed turns that carry no `fallback` content block) and stamps `ParsedResponse.served_model`; `track_daily_cost` bills at the served model's rates, `append_fallback_embed` surfaces a notice, and `extract_response_content` skips the `fallback` block type. The alternative client-side `BetaRefusalFallbackMiddleware` was deliberately not used — the bot is first-party-API only, so the one-round-trip server-side path is simpler.
