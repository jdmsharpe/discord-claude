...
- **Refusal fallback (server-side, beta):** when the requested model is in `REFUSAL_FALLBACK_MODELS` (`util.py`, currently `claude-fable-5`), `call_api_with_tool_loop` adds the `server-side-fallback-2026-06-01` beta and `fallbacks=[{"model": REFUSAL_FALLBACK_MODEL}]` (`claude-opus-4-8`; the explicit-list form accepts up to three named models, and a `fallbacks="default"` routing mode exists under the `server-side-fallback-2026-07-01` header — not adopted). Served-by detection reads `usage.iterations` for a `fallback_message` entry (covers sticky-routed turns that carry no `fallback` content block) and stamps `ParsedResponse.served_model`; `track_daily_cost` bills at the served model's rates, `append_fallback_embed` surfaces a notice, and `extract_response_content` skips the `fallback` block type. The alternative client-side `BetaRefusalFallbackMiddleware` was deliberately not used — the bot is first-party-API only, so the one-round-trip server-side path is simpler.
