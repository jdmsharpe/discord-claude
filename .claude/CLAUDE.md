...
- **Cost calculation**: `calculate_cost()` accounts for cache write tokens (2x base input price for 1h TTL), cache read tokens (0.1x base input price), and web search requests ($0.01/search via `WEB_SEARCH_COST_PER_REQUEST`) in addition to regular input/output tokens. The pricing embed shows cache hits, web searches, web fetches, and code executions when present
- **Cost logging**: `_track_daily_cost()` emits a structured `COST |` log line after every API call with user, model, all token counts, server tool use counts, per-request cost, and daily total. This persists via the Python logger (daily totals in `self.daily_costs` are still in-memory only)
- **Server tool tracking**: `ParsedResponse` tracks `web_search_requests`, `web_fetch_requests`, and `code_execution_requests` extracted from the API response's `usage.server_tool_use` object, accumulated across tool loop iterations
