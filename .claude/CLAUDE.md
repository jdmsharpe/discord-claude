...
- **Future migration target:** the SDK now ships `client.beta.messages.tool_runner` with `anthropic.lib.tools.mcp.async_mcp_tool` for first-class MCP integration. Once that surface is GA-promoted out of `beta`, retire the manual `mcp-client-2025-11-20` beta-header path in `call_api_with_tool_loop`. (Last checked anthropic 0.109.1 on 2026-06-10 — still beta-only: `client.messages.tool_runner` does not exist, only `client.beta.messages.tool_runner`. 0.108.0 added `claude-fable-5`/`claude-mythos-5` to the typed Model union plus refusal-fallback machinery: `BetaRefusalFallbackMiddleware`/`BetaFallbackState` client middleware and a server-side `fallbacks` param (beta `server-side-fallback-2026-06-01`, only target `claude-opus-4-8`) — candidate opt-in hardening now that the bot offers claude-fable-5, whose safety classifiers can return `stop_reason: "refusal"`. Recheck on next anthropic SDK minor bump.)
