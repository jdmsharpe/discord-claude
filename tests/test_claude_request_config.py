...
class TestToolChoiceSupport:
    ...
    def test_validate_request_configuration_rejects_advisor_outside_executor_pairs(self):
        ...
        Opus 5.5 / Opus 5 / Fable 5 only take Opus 5.5 / Opus 5 / Fable 5 / Fable 5.1
        advisors and Fable 5.1 takes only Fable 5.1 and Opus 5.5 advisors, so the
        otherwise default claude-opus-4-8 must be refused there; the mid-tier executors
    ...
    def test_validate_request_configuration_rejects_forced_tool_use(self, model):
        ...
        for tool_choice in ({"type": "any"}, {"type": "tool", "name": "memory"}):
            params = ChatCompletionParameters(
                model=model, tools=["memory"], tool_choice=tool_choice
            )
            error = ClaudeCog._validate_request_configuration(params)
            assert error is not None, tool_choice
            assert f"`{model}` does not support forced tool use" in error

        params = ChatCompletionParameters(
            model=model, tools=["memory"], tool_choice={"type": "auto"}
        )
        assert ClaudeCog._validate_request_configuration(params) is None
...
class TestThinkingDisplay:
    ...
    def test_validate_request_configuration_rejects_updates_display_elsewhere(self, model):
        ...
        assert "`claude-opus-5-5`" in error
